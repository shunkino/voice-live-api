...
- 「モデル」は **2 種類**あり、混同しやすい:
  1. **推論 LLM**（エージェントが内部で使う頭脳。env var で指定。今回の天気エージェントは mock で LLM 未使用）
...
└───┼──────────────────────────┼──────────────────────────────┼──────────────────┘
    ...
    └──────────────────▶│ Foundry ホスト型エージェント（コンテナ）            │
                        ...
                        │  @app.invoke_handler:                          │
                        │    input_audio.transcription → 天気ロジック       │
                        │    → output_audio_transcription.delta/.done     │
                        │  （任意で推論 LLM を env var で呼ぶ。今回は mock） │
                        └─────────────────────────────────────────────────┘
```
...
```jsonc
{
  "definition": {
    "kind": "hosted",
    "image": "voiceliveacrn3ko3.azurecr.io/weather-forecast-agent:v3",
    "cpu": "1", "memory": "2Gi",
    "container_protocol_versions": [
      { "protocol": "invocations",    "version": "1.0.0" },
      { "protocol": "invocations_ws", "version": "1.0.0" }
    ],
    "environment_variables": { "WEATHER_PROVIDER": "mock" }
  },
  "agent_endpoint": {
...
|---|---|---|---|
| 推論 LLM | エージェントの「頭脳」。文章の中身を生成 | エージェントの **env var**（例: `AZURE_AI_MODEL_DEPLOYMENT_NAME`）。コンテナ内で Responses API などを呼ぶ | **未使用**（`WEATHER_PROVIDER=mock` の決め打ちロジック） |
...
エージェントは（必要なら自分の LLM で）テキスト返答を作る → Voice Live がクライアント指定の声で読み上げる。
...
| エージェント側の処理 | `hosted-agents/weather-forecast/agent/server.py: handle_invoke` | `input_audio.transcription` → 天気ロジック → `output_audio_transcription.*` SSE |
| 推論 LLM（使うなら） | エージェントの env var | 例: `AZURE_AI_MODEL_DEPLOYMENT_NAME`（コンテナ内で参照） |
...
- 本リポジトリ: `hosted-agents/weather-forecast/`（エージェント本体）、`voicelive_demo/`（CLI/Web クライアント）
