...
class AudioProcessor:
    ...
    def _select_devices(self) -> tuple[Optional[int], Optional[int]]:
        ...
        ext_inputs = [(i, n) for i, n in input_candidates if not _is_builtin(n)]
        ext_outputs = [(i, n) for i, n in output_candidates if not _is_builtin(n)]
...
class BasicVoiceAssistant:
    ...
    async def _setup_session(self) -> None:
        ...
        #   latency_threshold_ms: SDK default is 2000ms. Tune based on observed latency.
        #     - 500-800ms: Aggressive — interim fires for most tool calls
        #     - 1000-1500ms: Balanced — only fires for slower operations
        #     - 2000ms+: Conservative — only fires for noticeably long waits
        interim_response_config = LlmInterimResponseConfig(
            ...
            instructions="""ツール呼び出しや処理の待ち時間中に、短く自然な中間応答を生成してください。
例:「少々お待ちください」「確認しております」など。
毎回の応答には含めないでください。ツール呼び出し時に「リアルタイムの情報にアクセスできません」とは言わないでください。"""
        )
